...
import { classifyServiceForMembership } from "@/lib/membership-engine.mjs";
...
export function isIvBagService(service = {}) {
  const name = `${service.name ?? service.displayName ?? ""}`.toLowerCase();
  const category = `${service.category ?? ""}`.toLowerCase();

  return /iv therapy/.test(category) && /drip/.test(name);
}

export function isBookingAddOnEligibleService(service = {}) {
  const name = `${service.name ?? service.displayName ?? ""}`.toLowerCase();
  const category = `${service.category ?? ""}`.toLowerCase();

  return (
    /injections|boosters/.test(category) ||
    /peptide wellness/.test(category) ||
    /nad\+?\s*injection\s*50mg/.test(name)
  );
}
...
export function buildBookableServices(catalog, catalogOverrides = {}) {
  const services = catalog.map((treatment) => {
    ...
    const classification = classifyServiceForMembership(baseService);

    return {
      ...baseService,
      membershipBucket: classification.bucket,
      membershipKind: classification.serviceKind,
      ivSizeMl: classification.ivSizeMl,
      isBucketOneEligible: classification.isBucketOneEligible,
      isStarterDripEligible: classification.isStarterDripEligible,
      isPremiumIv: classification.isPremiumIv,
      isStandardIv: classification.isStandardIv,
      isInjection: classification.isInjection,
      isPeptide: classification.isPeptide,
      isBotox: classification.isBotox,
      isIvBag: isIvBagService(baseService),
      isAddOnEligible: isBookingAddOnEligibleService(baseService),
      sizeSlug: getSizeSlug(treatment.name),
    };
  });
  const byBaseNameAndSize = new Map(
    services.map((service) => [`${service.baseName}:${service.sizeSlug ?? "none"}`, service]),
  );

  return services.map((service) => {
    const smallVariant = byBaseNameAndSize.get(`${service.baseName}:small`);

    return {
      ...service,
      smallVariantId: smallVariant?.id ?? null,
      smallVariantPrice: smallVariant?.price ?? null,
      hasSmallVariant: Boolean(smallVariant),
    };
  });
}
