...
import { getSlotAvailability, isConciergeLocation } from "@/lib/bookingRules";
...
export async function createUserBooking(user, bookingInput) {
  ...
  const totalPaid = Math.max(bookingInput.orderTotal - dripCredit, 0);
  const dripsEarned = calculateDripsEarned(totalPaid, rewards.tier);
  ...
  const nextTier = getTierForLifetimeDrips(nextLifetimeDrips);
  ...
  const existingBookings = await getBookingsForDate(bookingInput.appointmentDate);
  const slotAvailability = getSlotAvailability({
    bookings: existingBookings,
    date: bookingInput.appointmentDate,
    time: bookingInput.appointmentTime,
    locationType: bookingInput.location?.type,
  });

  if (!slotAvailability.available) {
    throw new Error(slotAvailability.reason || "This time slot is no longer available.");
  }
