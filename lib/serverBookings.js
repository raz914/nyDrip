...
function normalizeItems(items = []) {
  return items.map((item) => ({
    ...
    isIvBag: item.isIvBag ?? isIvBagService(item),
    isAddOnEligible: item.isAddOnEligible ?? isBookingAddOnEligibleService(item),
    isInjection: item.isInjection ?? false,
    isPeptide: item.isPeptide ?? false,
    smallVariantId: item.smallVariantId ?? null,
    smallVariantPrice: item.smallVariantPrice ?? null,
  }));
}

function assertBookingCartConstraints(items = []) {
  const ivBagCount = items.filter((item) => item.isIvBag || isIvBagService(item)).length;

  if (ivBagCount > 1) {
    throw new Error("Only 1 IV bag can be selected at a time.");
  }
}
...
function buildGuestBookingContext(bookingInput) {
  ...
  assertBookingCartConstraints(items);
