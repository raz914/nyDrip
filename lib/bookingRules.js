...
function isExpiredPendingPaymentBooking(booking) {
  if (booking.status !== "PendingPayment") {
    return false;
  }

  const expiresAt = toDate(booking.payment?.expiresAt);

  return Boolean(expiresAt && expiresAt.getTime() <= Date.now());
}

function isBlockingBookingStatus(booking) {
  return !["Cancelled", "Expired"].includes(booking.status) && !isExpiredPendingPaymentBooking(booking);
}
...
export function getBookingsForSlot(
  ...
  return bookings.filter((booking) => {
    if (booking.appointmentDate !== date || !isBlockingBookingStatus(booking)) {
      return false;
    }
